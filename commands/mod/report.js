...
module.exports.run = async (bot, message, args) => {
  ...
  .setColor("#999999")
  .setDescription("**Description:** Report a player to staff!\n**Usage:** /report [user] [reason]\n**Examples:**\n  /report @derpynoob133 V1a");
  
  
  let rUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
  if (!rUser) return message.channel.send(infoEmbed).then(msg => msg.delete(10000));
  let reason = args[1];
  ...
  if(!reason) return message.channel.send(infoEmbed).then(msg => msg.delete(10000));
  ...
  if (args[1] === null){
    ...
    return await message.channel.send(`${args[1]} isn't a valid rule! Please put the rule number and, or the type. Eg. /report @user V1 (V standing for Voice Rules, etc.)`).then(msg => msg.delete(5000));
  }
